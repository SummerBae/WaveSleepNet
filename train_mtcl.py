    ...
    def _diversity_cal(self, X):
        ...
        for i in range(num):
            if i >= num - 1:
                ...
                signal2 = weight[i+1:]
                # signal2 = signal2.view(-1, signal2.size()[1]).transpose(0, 1)
                signal2 = torch.squeeze(signal2, dim=2)
                ## shape [1, M-i]
                distance = list_of_distance(signal1, signal2)
                # diversity.extend(torch.min(distance, 1).values)
        ...
        for i, (inputs, labels) in enumerate(self.loader_dict['train']):
            ...
            if self.cfg['backbone']['name'] != 'ProtoP':
                for j in range(len(outputs)):
                    if self.cfg['classifier']['name'] == 'ProtoP':
            ...
            if self.train_iter % self.tp_cfg['val_period'] == 0:
                ...
                # if self.cfg['backbone']['name'] != 'AttnSleep':
                if self.cfg['backbone']['name'] != 'ProtoP':
                    self.activate_train_mode()
...
def main():
    ...
    # parser.add_argument('--config', type=str, help='config file path', default='/summer/ProtoPSleep/configs/SleePyCo-Transformer_SL-10_numScales-3_SHHS_wavesensing.json')
    parser.add_argument('--config', type=str, help='config file path', default='/summer/ProtoPSleep/configs/SleePyCo-Transformer_SL-10_numScales-3_SHHS_wavesensing_single_epoch.json')
